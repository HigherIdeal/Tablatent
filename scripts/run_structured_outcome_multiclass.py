...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--fold',type=int,default=2024);ap.add_argument('--seed',type=int,default=42);ap.add_argument('--devices',default='0');ap.add_argument('--ids',action='store_true');ap.add_argument('--anchors',action='store_true');ap.add_argument('--rich',action='store_true');ap.add_argument('--depth',type=int,default=8);ap.add_argument('--iterations',type=int,default=600);a=ap.parse_args()
 ...
 cross=offset_core.add_anchor_cross(f) if a.rich else [];match=offset_core.add_frozen_matchup(f) if a.rich else [];count=offset_core.add_frozen_count_profiles(f) if a.rich else [];pressure=offset_core.add_frozen_pressure_profiles(f) if a.rich else [];domain=offset_core.add_frozen_domain_profiles(f) if a.rich else [];auxall=multi_core.auxiliary_targets(f);auxprof=offset_core.add_frozen_aux_profiles(f,auxall,'pressure') if a.rich else []
 ...
 anchors=[] if not a.anchors else ['eng_anchor_available','eng_anchor_gap_n',*[x for short in ('success','reverse','middle','ball','strike') for x in (f'eng_anchor_{short}_rate',f'eng_since_anchor_{short}_rate',f'eng_since_anchor_{short}_minus_long')],*banchor,*cross,*match,*count,*pressure,*domain,*auxprof]
 ...
 model=CatBoostClassifier(iterations=a.iterations,learning_rate=.03,depth=a.depth,loss_function='MultiClass',l2_leaf_reg=20,random_strength=.5,bootstrap_type='Bayesian',bagging_temperature=.5,border_count=128,random_seed=a.seed,has_time=True,one_hot_max_size=10,allow_writing_files=False,task_type='GPU',devices=a.devices,verbose=False).fit(Pool(x,cls[keep],cat_features=c,feature_names=feats))
 yy=va.control_success.to_numpy(float);out=ROOT/f"outputs/structured_multiclass_f{a.fold}_s{a.seed}{'_ids' if a.ids else ''}{'_anchors' if a.anchors else ''}{'_rich' if a.rich else ''}_d{a.depth}_i{a.iterations}";out.mkdir(parents=True,exist_ok=True);saved={'y':yy,'gt':va.game_type.astype(str).to_numpy()}
 for t in (200,400,600,800):
  if t>a.iterations:continue
